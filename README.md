...
<details>
<summary>1. struct 和 class 有什么区别？（★★★★★）</summary>

**回答（口语版）：**

这个也是 Swift 面试里面比较经典的一道题。

我觉得 `struct` 和 `class` 最大的区别，其实就是**值类型和引用类型**。

`struct` 属于值类型，赋值或者作为参数传递的时候，会拷贝一份新的数据，所以修改新的对象，不会影响原来的对象。

而 `class` 属于引用类型，赋值的时候复制的是对象引用，也就是多个变量指向同一个对象，所以修改其中一个对象，其他引用看到的数据也会发生变化。

第二个区别就是内存管理。

`struct` 不参与 ARC 引用计数，所以也不会出现循环引用的问题。

而 `class` 是由 ARC 管理的，如果两个对象互相强引用，或者逃逸闭包强引用了对象，就有可能造成内存泄漏，所以平时开发中像异步回调，我都会根据情况使用 `[weak self]`。

第三个区别就是继承。

`struct` 不支持继承，而 `class` 支持继承、多态和方法重写，所以像 `UIViewController`、`UIView`、`NSObject` 这些系统类，都是基于 `class` 实现的。

还有一个区别就是可变性。

如果 `struct` 使用 `let` 定义，那么整个对象都不能修改，即使里面的属性是 `var` 也不行。

而 `class` 即使用 `let` 定义，只要属性本身是 `var`，还是可以修改属性值，因为 `let` 限制的是引用地址不能改变，而不是对象本身的数据。

实际开发里面，我一般会遵循 Apple 推荐的原则。

如果只是定义一个数据模型，比如用户信息、商品信息、配置数据这些，没有继承需求，也不需要多个地方共享同一份数据，我会优先使用 `struct`。

如果这个对象需要多个地方共享，或者需要继承，比如 `ViewController`、网络管理器、播放器、缓存管理器这些，我就会使用 `class`。

所以我的理解就是：

`struct` 更适合描述数据；`class` 更适合描述对象和生命周期管理。
...
<details>
<summary>2. 为什么 Apple 推荐优先使用 struct？（★★★★★）</summary>

**回答（口语版）：**

Apple 官方一直推荐 **"Prefer Structures Because They Are Safer"**。

主要有几个原因。

第一，`struct` 是值类型。

多个地方使用的时候，每个人拿到的都是自己的副本，不会因为一个地方修改数据影响另外一个地方，代码更加安全。

第二，不参与 ARC。

没有引用计数，也不会发生循环引用，所以不需要考虑 `weak`、`unowned` 等内存管理问题。

第三，性能更好。

Swift 对 `struct` 做了很多优化，比如 **Copy On Write**。

像 `Array`、`Dictionary`、`String` 虽然都是 struct，但是只有真正修改数据的时候才会复制，所以性能并不会因为值拷贝变差。

第四，更符合 Swift 的设计思想。

Swift 更强调值语义，让数据保持独立，减少共享状态带来的 Bug。

所以项目里面，只要没有继承需求，也不需要共享对象，我都会优先使用 `struct`。
...
<details>
<summary>3. struct 一定存储在栈上吗？class 一定存储在堆上吗？（★★★★★）</summary>
...
Swift 编译器会根据实际情况决定存储位置。

例如：

如果 `struct` 是局部变量，而且没有发生逃逸，一般会放到栈上。
...
<details>
<summary>4. let 修饰 struct 和 class 有什么区别？（★★★★★）</summary>
...
<details>
<summary>6. 项目中什么时候使用 struct？什么时候使用 class？（★★★★★）</summary>

**回答（口语版）：**

项目里面其实两种都会用，但是职责是不一样的。

一般来说，**数据模型我会使用 `struct`，业务管理对象我会使用 `class`。**
...
这些 Model，我都会使用 `struct`。

因为它只是保存数据，没有继承需求，也不需要多个地方共享同一个对象。
...
这些需要维护状态、多个地方共享资源的对象，我都会使用 `class`。
...
<details>
<summary>7. struct 为什么没有循环引用？（★★★★☆）</summary>
...
<details>
<summary>8. struct 可以继承吗？为什么不能继承？（★★★★☆）</summary>

**回答（口语版）：**

不能。

因为 `struct` 设计理念就是描述数据。
...
<details>
<summary>13. 什么是 Copy On Write（COW）？（★★★★★）</summary>

**回答（口语版）：**

COW，全称 **Copy On Write（写时复制）**，也是 Swift 面试特别高频的一道题。

它的意思就是：**多个变量共享同一份数据，只有真正修改的时候，才会复制一份新的数据。**
...
所以很多人觉得值类型性能差，其实 Swift 已经做了很多优化。
